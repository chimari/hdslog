...
int scp_write_remote(typHLOG *hl, gchar *result_file){
  int ret;
  FILE *fp;
  gchar *sftp_log;
  ...
  loclfile=g_strdup(result_file);
  ret=scp_w_spec(hl, loclfile, FALSE);
  g_free(loclfile);

  if(ret>=0){
    loclfile=g_strdup_printf("%s%s%s%shdslog-%04d%02d%02d.txt",
    ...
    ret=scp_w_spec(hl, loclfile, TRUE);
    g_free(loclfile);
  }

  sftp_log=g_strconcat("/tmp",
		       G_DIR_SEPARATOR_S,
		       SFTP_LOG,
		       NULL);
  ...
  if(ret<0){
    ...
    msg=g_strconcat("   Host : ",hl->remote_host, NULL);
    ...
    msg=g_strconcat("   User : ", hl->remote_user, NULL);
    label = gtk_label_new (msg);
#ifdef USE_GTK3
    gtk_widget_set_halign (label, GTK_ALIGN_START);
    gtk_widget_set_valign (label, GTK_ALIGN_CENTER);
    ...
    msg=g_strconcat("   Password : ", hl->remote_pass, NULL);
    label = gtk_label_new (msg);
#ifdef USE_GTK3
    gtk_widget_set_halign (label, GTK_ALIGN_START);
    gtk_widget_set_valign (label, GTK_ALIGN_CENTER);
...
#endif
		  POPUP_TIMEOUT,
		  "The reduced spectrum has been successfully uploaded.",
		  NULL);
    unlink(sftp_log);
    if(sftp_log) g_free(sftp_log);
    return(0);
...
int scp_write_cal(typHLOG *hl){
  ...
  for(i=0;i<NUM_SET;i++){
    // Ap Red
    if(hl->flag_ap_red[i]){
      loclfile=g_strconcat(hl->wdir,
			   G_DIR_SEPARATOR_S,
			   hl->ap_red[i],
			   ".fits",
			   NULL);
      ret=scp_w_main(hl, loclfile, FALSE);
      g_free(loclfile);
      if(ret<0) break;
      ...
      loclfile=g_strconcat(hl->wdir,
			   ...
			   hl->ap_red[i],
			   NULL);
      ret=scp_w_main(hl, loclfile, TRUE);
      g_free(loclfile);
      if(ret<0) break;
    }
  ...
  if(ret<0){
    ...
    msg=g_strconcat("   User : ", HDS01_UNAME, NULL);
    label = gtk_label_new (msg);
#ifdef USE_GTK3
    gtk_widget_set_halign (label, GTK_ALIGN_START);
    gtk_widget_set_valign (label, GTK_ALIGN_CENTER);
    ...
    msg=g_strconcat("   Password : ", hl->spass, NULL);
    label = gtk_label_new (msg);
#ifdef USE_GTK3
    gtk_widget_set_halign (label, GTK_ALIGN_START);
    gtk_widget_set_valign (label, GTK_ALIGN_CENTER);
...
#endif
		  -1,
		  "The CAL files have been successfully uploaded.",
		  NULL);
    unlink(sftp_log);
    if(sftp_log) g_free(sftp_log);
    return(0);
...
int scp_w_main(typHLOG *hl, gchar *loclfile, gboolean db_flag)
{
    ...
    gchar *username=g_strdup(HDS01_UNAME);
    gchar *password=g_strdup(hl->spass);
    gchar *scppath;
    FILE *local;
    int rc;
    char mem[1024];
    ...
    fname=g_path_get_basename(loclfile);
    if(db_flag){
      scppath=g_strconcat(hl->sdir,
			  G_DIR_SEPARATOR_S,
			  "database",
			  G_DIR_SEPARATOR_S,
			  fname,
...
int scp_w_spec(typHLOG *hl, gchar *loclfile, gboolean log_flag)
{
    struct hostent *servhost;         /* ホスト名とIPアドレスを扱うための構造体 */
    int sock, i, auth_pw = 1;
    struct sockaddr_in sin;
    ...
    LIBSSH2_SESSION *session = NULL;
    LIBSSH2_CHANNEL *channel;
    gchar *username=g_strdup(hl->remote_user);
    gchar *password=g_strdup(hl->remote_pass);
    gchar *scppath;
    FILE *local;
    int rc;
    char mem[1024];
    ...
    if(log_flag){
      scppath=g_strconcat(hl->remote_dir,
			  G_DIR_SEPARATOR_S,
			  "README.txt",
			  NULL);
    }
    ...
    servhost = gethostbyname(hl->remote_host);
    if ( servhost == NULL ){
        fprintf(fp, "Bad hostname [%s]\n", hl->remote_host);
	err_flag=TRUE;
	fclose(fp);
